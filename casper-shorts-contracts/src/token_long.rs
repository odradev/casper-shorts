...
impl TokenLong {
    ...
    pub fn transfer(&mut self, recipient: &Address, amount: &U256) {
        ...
        if pack.is_wcspr_token(&recipient) {
            self.cfg.market().withdraw_long_from(&sender, *amount);
        } else {
...
mod tests {
    ...
    fn it_works() {
        ...
        // Autogenerated `TokenLongInitArgs` implements `InitArgs` trait
        // and `TokenLongHostRef` implements `Deployer` trait,
        // so we can use it to deploy the module.
        let init_args = TokenLongInitArgs {
            name: "TokenLong".to_string(),
            symbol: "TL".to_string(),
            decimals: 10,
            initial_supply: U256::from(1_000_000_000_000u64),
        };
        assert!(TokenLongHostRef::try_deploy(&env, init_args).is_ok());
    }
}
