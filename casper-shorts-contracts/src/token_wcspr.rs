...
mod tests {
    ...
    fn it_works() {
        ...
        // Autogenerated `TokenWCSPRInitArgs` implements `InitArgs` trait
        // and `TokenWCSPRHostRef` implements `Deployer` trait,
        // so we can use it to deploy the module.
        let init_args = TokenWCSPRInitArgs {
            name: "TokenWCSPR".to_string(),
            symbol: "TS".to_string(),
            decimals: 10,
            initial_supply: U256::from(1_000_000_000_000u64),
        };
        assert!(TokenWCSPRHostRef::try_deploy(&env, init_args).is_ok());
    }
}
